...
const CssStyle* CssParser::getStyleForClass(const std::string& className) const {
  ...
  if (it != styleMap_.end()) {
    // v2.0.181 — StyleEntry replaces the old pair<string, CssStyle>;
    // member is `style` instead of `second`.
    return &it->style;
  }
  return nullptr;
}
  ...
  while (start < len) {
    ...
    if (!singleSelector.empty()) {
      ...
      if (style.hasTextAlign || style.hasFontStyle || style.hasFontWeight || style.hasDirection) {
        // v2.0.181 — find/insert into sorted styleMap_ via arena-slice
        // comparison.  Find = binary search; on hit, merge into existing
        // StyleEntry; on miss, append the selector chars to arena_ and
        // insert a new StyleEntry pointing at that arena slice.
        ...
        const bool isMatch =
            insertPos != styleMap_.end() &&
            compareSliceToKey(*insertPos, singleSelector.data(), singleSelector.size()) == 0;
        if (isMatch) {
          insertPos->style.merge(style);
        } else if (styleMap_.size() < MAX_CSS_RULES) {
          // Bounds check on arena offset: the encoding uses uint16_t for
          // both offset and length, so the arena can hold at most 65535
          // characters before we'd overflow.  In practice CSS selectors
          // are <30% of the source file and MAX_CSS_FILE_SIZE = 64 KB,
          // so we sit ~10-20 KB inside the limit — but defend against
          // pathological inputs (autogenerated CSS, very long selectors).
          if (arena_.size() + singleSelector.size() > 0xFFFFu) {
            LOG_DBG(TAG, "CSS arena would overflow at selector '%s' — skipping rule",
                    singleSelector.c_str());
          } else {
            const uint16_t offset = static_cast<uint16_t>(arena_.size());
            const uint16_t length = static_cast<uint16_t>(singleSelector.size());
            arena_.insert(arena_.end(), singleSelector.begin(), singleSelector.end());
            styleMap_.insert(insertPos, StyleEntry{offset, length, style});
          }
        }
      }
    }
