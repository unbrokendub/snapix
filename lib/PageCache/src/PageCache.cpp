...
namespace {
constexpr uint8_t CACHE_FILE_VERSION = 32;  // v32: v3.2.0 scene-break ornament ("* * *") consumes an extra line at each <hr>, shifting page layout/counts; rebuild caches built under v31.  // v31: v3.1.0 "красная строка" first-line paragraph indent.  // v30: v2.0.206 off-by-one page-count fix.  The streaming short-circuit (R4.b) now emits `boundaryCount + 1` Pages instead of `boundaryCount` — the final partial page of every multi-page chapter/section was previously never created (the MEASURE walk fires boundaries only on overflow rolls, so page 0 and the EOF-terminated last page are implicit).  Existing v29 page caches were built with the short count and would keep dropping each chapter's tail; bumping forces a rebuild through the fixed parser so the recovered last page is materialised.  (v29: v2.0.60 page cache moved from SD to LittleFS.)
