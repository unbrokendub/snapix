...
// v3.10.4 — REVERTED v3.10.3's 72 KB/36 KB raise back to 56 KB/24 KB.  v3.10.3
// raised the gate to avoid a heap dip to ~5 KB during a far-sweep of a big
// chapter — but the device's steady-state free while reading a first-time EPUB
// is only ~57 KB, so a 72 KB gate stopped the read-ahead from EVER running →
// "Loading…" appeared at every new chapter (and exposed a display glitch behind
// the banner).  Crucially the dip it was avoiding is OOM-SAFE: the far-sweep
// markerize runs inside parsePages under PageCache::create/extend's
// try/catch(std::bad_alloc), so a true OOM degrades to "partial cache, retry",
// never a reboot.  There is no static gate that both runs at ~57 KB steady AND
// avoids the dip (one big chapter's markerize needs ~50 KB), so we choose
// seamless read-ahead + an OOM-safe dip over needless "Loading" pauses.
...
bool ReaderCacheController::prefetchNextEpubSpineCache(Core& core, const RenderConfig& config, const int activeSpineIndex,
  ...
  bool didWork = false;
  bool sweepComplete = allowFarSweep;
  ...
  if (allowFarSweep) {
    backgroundPrefetchSweepComplete_ = sweepComplete;
    if (sweepComplete) {
      backgroundPrefetchCursorSpine_ = -1;
    }
  }
